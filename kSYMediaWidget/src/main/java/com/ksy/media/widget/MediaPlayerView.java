...
public class MediaPlayerView extends RelativeLayout {
	...
	private void init(Context context, AttributeSet attrs, int defStyle)
		...
		mMediaPlayerSmallControllerView.mBack_Llayout
		.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				case 0:
					// 请求窗口
					requestPlayMode(1);
					mPlayerViewCallback.exitMediaPlayer(0);
					break;
				case 1:
					// 窗口
					mPlayerViewCallback.exitMediaPlayer(1);
					// Toast.makeText(getContext(), "退出", 200).show();
					break;
				default:
					break;
				}
	 ...
	 *            视频名称
	 */
	public void play(String path, String videoTitle) {
	...
	public boolean requestPlayMode(int requestPlayMode) {
		...
		if (MediaPlayerUtils.isFullScreenMode(requestPlayMode)) {
			...
			mPlayerViewCallback.isFullScreenMode(true, requestPlayMode);
			return true;
		...
		// 请求窗口模式
			...
			mPlayMode = requestPlayMode;
			mPlayerViewCallback.isWindowMode(true, requestPlayMode);
			return true;
	...
	private void updateVideoInfo2Controller() {
		// 设置视频名称
		mMediaPlayerSmallControllerView.updateVideoTitle(VideoTitle);
		
		mMediaPlayerLargeControllerView.updateVideoTitle(VideoTitle);
		...
		mMediaPlayerEventActionView.updateVideoTitle(VideoTitle);
	}
	...
	public interface PlayerViewCallback {
		...
		/** 全屏模式播放 */
		void isFullScreenMode(boolean isFullScreenMode, int code);

		/** 窗口模式播放 */
		void isWindowMode(boolean isWindowMode, int code);

		/** 退出播放器 ,code为0，转为窗口模式播放，code为1，退出播放器*/
		void exitMediaPlayer(int code);
	}
